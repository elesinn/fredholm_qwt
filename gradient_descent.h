...
QVector<double> GradDown(QVector<double> lambda, QVector<double> s, double alpha, double a, double b, QVector<double> u,QVector<double> delta)
{
    ...
    do
    {
        ...
        {
                    case 1:
                        beta = zoloto(0,100, grad, curr, s, alpha, u,delta);
                    case 2:
                        beta=step(curr,grad,s,a,b,alpha,u,delta);
                    case 3:
                        beta = diht(0, 10, grad, curr, s, alpha, u,delta);
        }
        curr = New(curr, grad, beta);
        it++;
    } while   (fabs(R(last, s, alpha, a, b, u,delta) - R(curr, s, alpha, a, b, u,delta))>eps);
  //  cout << "iter=" << it << endl;
    return curr;
